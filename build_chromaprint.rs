...
pub fn build() {
    ...
    if is_windows {
        ...
        println!("cargo:rustc-link-search=native={}", root_path.join("resources").join("lib").join("x64").display());
    } else {
        // For other platforms
        println!("cargo:rustc-link-search=native={}", root_path.join("resources").display());
    }
    ...
    println!("cargo:include={}", root_path.join("resources").join("include").display());
