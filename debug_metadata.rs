...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    dprintln!("=== Testing metadata preservation ===");
    
    // Test 1: Check if input file has metadata
    dprintln!("1. Checking input file metadata...");
    let codec = get_codec(input_file)?;
    let metadata = codec.extract_metadata_from_file(input_file)?;
    
    match &metadata {
        Metadata::Wav(chunks) => {
            dprintln!("Found {} metadata chunks in input file:", chunks.len());
            for (i, chunk) in chunks.iter().enumerate() {
                match chunk {
                    MetadataChunk::TextTag { key, value } => {
                        dprintln!("  [{}] Text: {} = {}", i, key, value);
                    }
                    MetadataChunk::Bext(_) => {
                        dprintln!("  [{}] BEXT chunk", i);
                    }
                    MetadataChunk::IXml(_) => {
                        dprintln!("  [{}] iXML chunk", i);
                    }
                    _ => {
                        dprintln!("  [{}] Other: {}", i, chunk.id());
                    }
                }
            }
        }
        _ => dprintln!("No WAV metadata found"),
    }
    ...
    dprintln!("Codex metadata has:");
    match &codex.metadata {
        Metadata::Wav(chunks) => {
            dprintln!("  {} chunks loaded into Codex", chunks.len());
        }
        _ => dprintln!("  No metadata in Codex"),
    }
    ...
    match &output_metadata {
        Metadata::Wav(chunks) => {
            dprintln!("Found {} metadata chunks in output file:", chunks.len());
            for (i, chunk) in chunks.iter().enumerate() {
                match chunk {
                    MetadataChunk::TextTag { key, value } => {
                        dprintln!("  [{}] Text: {} = {}", i, key, value);
        ...
        _ => dprintln!("No WAV metadata found in output"),
    }
